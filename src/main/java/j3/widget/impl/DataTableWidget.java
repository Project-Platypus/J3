...
public class DataTableWidget extends TitledWidget<DataTableWidget> {
	...
	private ObjectProperty<Instance> selectedInstance = new ObjectPropertyBase<Instance>() {
		...
		protected void invalidated() {
			Instance instance = selectedInstance.get();
			table.getSelectionModel().select(instance);

			if (instance == null) {
				target(null);
			} else {
				table.scrollTo(instance);

				if (getScene() != null) {
					for (Shape3D shape : Selector.on(getScene()).get(Shape3D.class)) {
						if ((shape.getUserData() instanceof Instance) && (shape.getUserData().equals(instance))) {
							target(shape);
							break;
						}
					}
