...
public abstract class ValueAxis<T extends Number> extends Axis<T> {
    ...
    public ValueAxis() {
        ...
        axisBody.getChildren().add(minorTickPath);
    }
    ...
    @Override protected void layoutChildren() {
        ...
        if (minorTickLength > 0 && length > 2 * getTickMarks().size()) {
            ...
            if (Side.LEFT.equals(side)) {
                ...
                minorTickPath.setStroke(Color.BLACK);
                minorTickPath.setStrokeWidth(2.0);
                for (int i = 0; i < minorTickMarkValues.size(); i += stripFactor) {
                    ...
                    if (y >= 0 && y <= length) {
                        minorTickPath.getElements().addAll(
                                new MoveTo(getWidth()/2 - minorTickLength/2, y),
                                new LineTo(getWidth()/2 + minorTickLength/2 - 1, y));
                    }
                }
